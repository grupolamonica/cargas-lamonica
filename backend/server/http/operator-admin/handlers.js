...
import { recordSecurityAuditEvent } from "../../lib/security-audit.js";
...
import {
  ...
  routeMutationSchema,
} from "../../services/operator-admin/schemas.js";
import {
  createOperatorCargo,
...
} from "../../services/operator-admin/read-models.js";
import { fetchDriverFlowMetrics } from "../../services/operator-admin/driver-flow-metrics.js";
import {
  ForbiddenError,
  LoadClaimServiceError,
  UnauthorizedError,
  ValidationError,
} from "../../services/load-claims/errors.js";
import { assertOperatorAccessLevel, assertOperatorPermission, hasOperatorPermission } from "../../services/load-claims/operator-access.js";
import { requireOperatorSession } from "../../services/load-claims/auth.js";
import { createSupabaseAdminClient, syncGoogleSheetLoads } from "../../services/google-sheet-loads.js";
  ...
  return withOperatorSession(request, "sheet-monitor", async ({ correlationId }) => {
    ...
    if (refresh) {
      const { getSheetExportUrl, fetchGoogleSheetCsv, updateSheetMonitorSnapshot } =
        await import("../../services/google-sheet-loads.js");
    ...
    const { getSheetExportUrl: getUrl } = await import("../../services/google-sheet-loads.js");
    return {
      statusCode: 200,
      payload: {
        items: [],
...
export async function resolveSheetMonitorRowDetailResponse(request) {
  return withOperatorSession(request, "sheet-monitor-row-detail", async ({ correlationId }) => {
    ...
    for (const plate of missingPlates) {
      try {
        const { lookupAngelliraPlate } = await import("../../services/driver-validation/angellira-client.js");
...
export async function resolveSheetMonitorEnrichResponse(request) {
  return withOperatorSession(request, "sheet-monitor-enrich", async ({ correlationId }) => {
    ...
    const supabaseClient = createSupabaseAdminClient();
    const { enrichSheetMonitorRows } = await import("../../services/operator-admin/sheet-monitor-enrichment.js");
