...
export async function updateProgramacaoSettings({ patch = {}, operatorId = null, deps = {} } = {}) {
  ...
  await run(async (client) => {
    ...
    if (Array.isArray(patch.alertRouteKeys)) {
      try {
        // DEVE ser o último statement da txn: em erro o pg aborta a txn e o COMMIT vira
        // ROLLBACK. Se a migration do DC-279 ainda não rodou (coluna ausente), toleramos
        // em vez de estourar 500 ao salvar as rotas de alerta (review DC-279 #6).
        await client.query(
          `UPDATE public.programacao_settings
